...
%      
 ...
 % Reconstuctution of primary states in the physical space    
     ...
     bc{1}=zeros(size(Tu,1));
     end

     if ~isempty(Tu.Ry0) 
     bc{2}=Tu.Ry0*uvec;
     else
     bc{2}=zeros(size(Tu,1));
     end

     if ~isempty(Tu.R20) 
     bc{3}=Tu.R20*uvec;
     else
     bc{3}=zeros(size(Tu,1));
     end
     ...
     coeff=flipud(coeff);
     for n=1:size(coeff,2);
     bcu_input(:,n+nshift(k))=polyval(coeff(:,n),grid.comp);
     end
     else % bc{k}.maxdeg=0 --> scalar entry
     for n=1:length(bc{k})
         if (k<3)
     bcu_input{n+nshift(k)}(1:length(grid.comp),:)=double(bc{k}(n));
         else
     bcu_input{n+nshift(k)}(1:length(grid.comp),1:length(grid.comp))=double(bc{k}(n));
         end
     end
     end % isa(bc{k},'polynomial') & bc{k}.maxdeg>0)
     end %k=1:3
