...
function [A, A_nonsquare]=PIESIM_1Dto2D2Mat_cheb(N, Rop, var1, p, p1, dir)
...
% Treatment of mutliplicative operator (opmult stands for multiplicative)
   ...
   if (~isempty(R0)) 
       [A_opmult, A_opmult_nonsquare]=PIESIM_PI2Mat_cheb_opmixed_discretize_1to2(N, R0, var1, p, p1, dir, 0);
   end

% Treatment of integrative operators 
% 
    if (~isempty(R1)) 
        [A_opint_block_1,A_opint_block_nonsquare_1]=PIESIM_PI2Mat_cheb_opmixed_discretize_1to2(N, R1, var1, p, p1, dir, [1 0]);
    end
% 
    if (~isempty(R2)) 
        [A_opint_block_2,A_opint_block_nonsquare_2]=PIESIM_PI2Mat_cheb_opmixed_discretize_1to2(N, R2, var1, p, p1, dir, [0 1]);
    end
