...
function solcoeff=PIESIM_int_symbolic(psize, opts, uinput, coeff, Dop)
    ...
    % (not necessarily polynomial)
              for i=1:nf    
              intsym=int(exp(lambda*(tt-st))*tforce(i),st,[0,tt]);
              force_contr(j,i)=double(subs(intsym,tt,tf));
              end 
           end    % for j loop
   ...
   % columns of inhom array that are non-zero
       ...
       for i=1:nf
           addition_symbolic=addition_symbolic+diag(force_contr(:,i))*inv(V)*Mcheb_inv*acheb_force(:,i);
       end
