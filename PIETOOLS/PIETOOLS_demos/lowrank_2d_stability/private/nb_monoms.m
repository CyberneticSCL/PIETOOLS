% nb_monoms.m -- VERBATIM copy of the private build_monoms subfunction from
% lpi_programming/poslpivar.m (lines 510-583) and lpi_programming/2D/poslpivar_2d.m
% (lines 2139-2213).  The two copies in the repo are byte-identical apart from a
% comment-whitespace difference on one line, so one copy serves both dimensions.  Copied, not reimplemented,
% so the basis counts reported here are the counts the solver actually sees.
% Lint is suppressed rather than "fixed": e.g. str2num on dec2bin(...) returns
% a VECTOR, which str2double would silently turn into NaN.
%#ok<*ST2NM,*ISCLSTR>
function Z = nb_monoms(vartable,maxdegs)
