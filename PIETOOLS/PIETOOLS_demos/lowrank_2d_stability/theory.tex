...
restricted SDP then repairs the point exactly within that subspace, and the
...
\item[Trace minimization.] The standard convex surrogate for low rank
  ...
  relied on.
\item[Structure: replication and transfer.] When the problem has replicated
  states, Section~6 removes the guess entirely: the $n=1$ face is
  constructed, not searched, at every larger $n$. More generally one can
  warm-start the descent from the face of a \emph{neighboring} problem (a
  smaller basis, a nearby parameter value). Measured support is partial:
  transfer up the basis axis was used successfully as a warm start, while
  naive transfer between differently-pruned settings failed in 1-D
  experiments --- treat cross-problem transfer as an initialization, never as
  a certificate.
