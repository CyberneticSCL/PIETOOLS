function [PDE_t] = PIETOOLS_PDE_Ex_2D_ReactionDiffusion_DDDD(GUI,params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% %     Dirichlet-Dirichlet Dirichlet-Dirichlet BCs
...
% %     mu_{m,n} = nu*pi^2*(m^2/(b-a)^2 +n^2/(d-c)^2)
...
M = 10;     N = 10;
Lx = b-a;   Ly = d-c;
r_lim = nu*pi^2*(1/Lx^2 +1/Ly^2);       % Stable iff r<=r_lim

mu_mn = nu*pi^2*(((2*(1:M)'-1)/Lx).^2 +((2*(1:N)-1)/Ly).^2);
