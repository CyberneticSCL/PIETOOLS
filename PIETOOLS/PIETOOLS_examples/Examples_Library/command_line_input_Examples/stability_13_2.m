...
%% Define equations
eq_PDE = [diff(x,t,2)==-alp/bet*diff(x,s,4);]; %  %   PDE: x1_{t} = -alp/bet*x2_{ssss},
           %         diff(x2,t,1)==x1;]; %             x2_{t} = x_1           |
eq_BC = [subs(x,s,0)==0; subs(diff(x,s,2),s,1)==0;%%   BCs: x2(s=0) = 0,        x2_{ss}(s=1) = 0      |
                subs(diff(x,s,1),s,0)==0; subs(diff(x,s,3),s,1)==0;];%        x2_{s}(s=0)= 0        x2_{sss}(s=1)= 0    |
