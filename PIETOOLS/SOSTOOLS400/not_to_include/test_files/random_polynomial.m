function [P] = random_polynomial(pvars,deg,nt,Degdensity,Cdensity,mdim,ndim)
% This function builds a random polynomial based on the following inputs:
%
...
if Cdensity>=1
    C = rand(nterms,mdim*ndim);
else
    C = sprand(nterms,mdim*ndim,Cdensity);
end

% Construct the dpvar
P = polynomial(C,degmat,pvarname,matdim);