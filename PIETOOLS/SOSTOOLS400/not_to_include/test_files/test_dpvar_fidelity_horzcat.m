% Code testing proper functioning of horzcat
...
if test==0  % run through (almost) all the tests
    ...
    testm2_array = (1:4)';
    testp2_array = (1:9)';
    test_array = [repmat(test1_array,(4*4*9),1),...
                  repmat(kron(test2_array,ones(8,1)),4*9,1),...
                  ones(8*4*4*9,1),...
                  repmat(kron(kron(testm2_array,ones(8,1)),ones(4,1)),9,1),...
                  kron(kron(kron(testp2_array,ones(8,1)),ones(4,1)),ones(4,1))];
    %test_array_m2 = [test1_array,ones(8,1),ones(8,1),4*ones(8,1),ones(8,1)];
    %test_array = [test_array; test_array_m2];
    ntests = 10;
else % run through some test case
    test_array = [5,1,1,2,1];
    ntests = 100;
end
...
if iseq_pvar
    ...
    np2 = length(pvars2);
elseif np_max2>0
    pvars2 = unique(randi(3*np_max2,[1,nd_max2]),'stable');      % indices of decision variables
    if isneq_pvar
...
% % % Concatenate and compare to polynomial operation % % %
cat1 = [D1,D2];
...
cat1_poly = [D1_poly,D2_poly];
try 
    error0 = double(dpvar2poly(cat1) - cat1_poly);
catch
...
cat2 = [D2,D1];
cat2_poly = [D2_poly,D1_poly];
try 
    error0 = double(dpvar2poly(cat2) - cat2_poly);
catch
...
cat3 = [D1,D2,D1,D2];
cat3_poly = [D1_poly,D2_poly,D1_poly,D2_poly];
try 
    error0 = double(dpvar2poly(cat3) - cat3_poly);
catch
    error('[D1,D2,D1,D2] does not produce the desired polynomial')
end
if norm(error0)~=0
    error('[D1,D2,D1,D2] does not produce the desired polynomial')
end
...
cat4 = [D2,D1,D1,D2];
cat4_poly = [D2_poly,D1_poly,D1_poly,D2_poly];
try 
    error0 = double(dpvar2poly(cat4) - cat4_poly);
catch
    error('[D2,D1,D1,D2] does not produce the desired polynomial')
end
if norm(error0)~=0
    error('[D2,D1,D1,D2] does not produce the desired polynomial')
end
...
disp('No errors were encountered testing horzcat!')