% Code testing proper functioning of subsasgn
...
% % % Use subsasgn and compare to polynomial result % % %
...
rindcs1 = 1:mdim1;          cindcs1 = 1:ndim1;
rindcs2 = randperm(mdim1);  cindcs2 = randperm(ndim1);
nr = randi(mdim1);          nc = randi(ndim1);
rindcs3 = rindcs2(1:nr);    cindcs3 = cindcs2(1:nc);

asgn1 = D1;
asgn1(rindcs1,cindcs1) = D2(rindcs1,cindcs1);
asgn1_poly = D1_poly;
asgn1_poly(rindcs1,cindcs1) = D2_poly(rindcs1,cindcs1);
error1 = dpvar2poly(asgn1) - asgn1_poly;
if max(error1.coeff,[],'all') >= tol
    error('subsagn with all indices does not produce the desired polynomial')
end

asgn2 = D1;
asgn2(rindcs2,cindcs2) = D2(rindcs2,cindcs2);
asgn2_poly = D1_poly;
asgn2_poly(rindcs2,cindcs2) = D2_poly(rindcs2,cindcs2);
error2 = dpvar2poly(asgn2) - asgn2_poly;
if max(error2.coeff,[],'all') >= tol
    error('subsagn with permuted indices does not produce the desired polynomial')
end

asgn3 = D1;
asgn3(rindcs3,cindcs3) = D2(rindcs3,cindcs3);
asgn3_poly = D1_poly;
asgn3_poly(rindcs3,cindcs3) = D2_poly(rindcs3,cindcs3);
error3 = dpvar2poly(asgn3) - asgn3_poly;
if max(error3.coeff,[],'all') >= tol
    error('subsagn with random indices does not produce the desired polynomial')
end

% asgn4 = D1;
% asgn4(rindcs3) = D2(rindcs3);
% asgn4_poly = D1_poly;
% asgn4_poly(rindcs3) = D2_poly(rindcs3);
% error4 = dpvar2poly(asgn4) - asgn4_poly;
% if max(error3.coeff,[],'all') >= tol
%     error('subsagn with linear indexing does not produce the desired polynomial')
% end

asgn5 = D1;
asgn5(end,end) = D2(end,end);
asgn5_poly = D1_poly;
asgn5_poly(end,end) = D2_poly(end,end);
error5 = dpvar2poly(asgn5) - asgn5_poly;
if max(error5.coeff,[],'all') >= tol
    error('subsagn with ''end'' indexing does not produce the desired polynomial')
end

asgn6 = D1;
asgn6([rindcs3,rindcs3],[cindcs3,cindcs3]) = D2([rindcs3,rindcs3],[cindcs3,cindcs3]);
asgn6_poly = D1_poly;
asgn6_poly([rindcs3,rindcs3],[cindcs3,cindcs3]) = D2_poly([rindcs3,rindcs3],[cindcs3,cindcs3]);
error6 = dpvar2poly(asgn6) - asgn6_poly;
if max(error6.coeff,[],'all') >= tol
    error('subsagn with indices exceeding size does not produce the desired polynomial')
end
...
asgn7_poly = D1_poly;
asgn7_poly(:,:) = D2_poly(:,:);
error7 = dpvar2poly(asgn7) - asgn7_poly;
if max(error7.coeff,[],'all') >= tol
    error('subsagn with '':'' indexing does not produce the desired polynomial')
end


% Excluding the following tests, since multipoly does not allow empty
% assignment
% asgn8 = D1;
% asgn8(:,1) = [];
% asgn8_poly = D1_poly;
% asgn8_poly(:,1) = [];
% error8 = dpvar2poly(asgn8) - asgn8_poly;
% if max(error8.coeff,[],'all') >= tol
%     error('assigning empty columns does not produce the desired polynomial')
% end
% 
% asgn9 = D1;
% asgn9(1,:) = [];
% asgn9_poly = D1_poly;
% asgn9_poly(1,:) = [];
% error9 = dpvar2poly(asgn9) - asgn9_poly;
% if max(error9.coeff,[],'all') >= tol
%     error('assigning empty rows does not produce the desired polynomial')
% end
...
disp('No errors were encountered testing subsasgn!')