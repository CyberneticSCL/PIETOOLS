function PDE_out = mtimes_pdes(PDE1,PDE2)
% PDE_OUT = MTIMES_PDES(PDE1,PDE2) computes the product of two terms that
% may appear in a PDE.
%
% INPUTS
% - PDE1:   'pde_struct' object representing a single term to be used in a
%           PDE;
% - PDE2:   'pde_struct' object representing a single term to be used in a
%           PDE;
%
% OUTPUTS
% - PDE_out:    'pde_struct' object representing the product of the two
%               input terms;
...
elseif any( eq_size_list1(eq_size_list2>1)>1 & eq_size_list1~=eq_size_list2 )
...
for i=1:numel(PDE_out.free)
    ...
    for j=1:numel(eq_i.term)
        ...
        eq_i.term = times_term(eq1_i.term{trm_num1},eq2_i.term{trm_num2},PDE_out);
    end
    PDE_out.free{i} = eq_i;
end
...
% Return a structure representing the product of two terms in a PDE

% Check whether term1 involves a state or input variable
if isfield(term1,'x')
    obj1 = 'x';
elseif isfield(term1,'u')
    obj1 = 'u';
elseif isfield(term1,'w')
    obj1 = 'w';
else
    error("Multiplication with output signals is not supported.")
end
obj1_num = term1.(obj1);
...
if isfield(term2,'x')
    obj2 = 'x';
elseif isfield(term2,'u')
    obj2 = 'u';
elseif isfield(term2,'w')
    obj2 = 'w';
else
    error("Multiplication with output signals is not supported.")
end
...
% Add potential missing object field to each term
if ~strcmp(obj1,obj2)
    term1.(obj2) = [];
    term2.(obj1) = [];
end
