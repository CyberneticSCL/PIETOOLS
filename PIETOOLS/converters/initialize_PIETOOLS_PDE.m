...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function PDE_out = initialize_PIETOOLS_PDE(PDE)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Batch input format is passed on to corresponding initializer.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2D systems are passed on to 2D version of the code.
% WARNING: 2D implementation is still under construction.
if all(size(PDE.n.n_pde)>=2) || (isfield(PDE.n,'Ndim') && PDE.n.Ndim==2)
    PDE_out = initialize_PIETOOLS_PDE_2D(PDE);
    return
end
...
if ~isfield(PDE.PDE, 'Crp')
    ...
    PDE.PDE.Crp = initCrp;
    if dispVal
        disp('Some of the terms in PDE output structure Crp have incorrect dimension or missing. Defaulting them to zero');
    end
end
...
if ~isfield(PDE.BC, 'Ebp')
    ...
    PDE.BC.Ebp = initEbp;
    if dispVal
        disp('Some of the terms in PDE BC structure Ebp have incorrect dimension or missing. Defaulting them to zero');
    end
end
