...
if ~singularET
    ...
    Vop_out = Aop_ODE(no+nw+nu+1:end, 1:end-nr);
    ...
    opvar Top_vbd;
    Top_vbd.dim = [v+nBVs nv;np_all_der_full np]; 
    ...
    opvar Ixwuf; Ixwuf.dim = [no+nz+ny+nv no+nw+nu; 0 np];
    Ixwuf.P = eye(no+nw+nu);

    PbigO = Aop_ODE*(Ixwuf+...
        [zeros(no,nr); zeros(nw,nr); zeros(nu,nr); eye(nr)]*PbigP(1:nr,:));
