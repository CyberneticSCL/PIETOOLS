%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS_Hinf_gain_dual_2D.m     PIETOOLS 2022a
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [prog, Pop, gam, solve_val] = PIETOOLS_Hinf_gain_dual_2D(PIE, settings, gain)
% This function use the KYP lemma in dual form to compute an upper bound
% on the H-infty gain a 2D-PIE system of the form
%
% T_op \dot{x}(t) = A_op  x(t) + Bw_op  w(t)
%            z(t) = Cz_op x(t) + Dzw_op w(t),
%
% by computing an upper bound for the dual of this system:
%
% T_op* \dot{x}(t) = A_op*  x(t) + Cz_op*  z(t)
%             w(t) = Bw_op* x(t) + Dzw_op* z(t)
%
...
if ~(Twop==0)
    error('The PIE takes the derivative dw/dt as input; dual H_infty gain analysis is not supported.')
end
...
if gain==0
    ...
    fprintf(['\n --- Testing the Hinf gain bound gam = ',num2str(gain),' using the dual KYP lemma --- \n'])
...
if norm(prog.solinfo.info.feasratio-1)<=.3 && ~prog.solinfo.info.numerr && ~prog.solinfo.info.pinf && ~prog.solinfo.info.dinf
    ...
    if ~isreal(gam)
        disp(double(lpigetsol(prog,gam))); % check the Hinf norm, if the problem was solved successfully
    else 
        disp(gam);
    end
