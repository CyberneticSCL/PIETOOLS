%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS_FEPIE2PDEstability.m     PIETOOLS 2025
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script executes a finete energy FE PIE2PDE stability analysis for 4-PIE System defined
...
%
% Stability implies existence of a C>0 such that \int_0^\infty|Tx(t)|^2dt \le C|x(0)|, which
% is weaker notion of stability than FE PDE stability. However, this script
% is slightly more computationally intensive than PIETOOLS_PDEstability.
%
% If settings.epneg>0, this test implies \int_0^\infty|Tx(t)|^2dt\le Ce^{-\alpha t}|x(0)|
% for some \alpha>0
%
% This script implements the LPI in Lemma 10, 1) of [1].
%
% INPUT: 
...
function [prog, P] = PIETOOLS_FEPIE2PDEstability(PIE, settings)
...
if PIE.dim==2
    ...
    if nargin==1
        [prog, P] = PIETOOLS_FEPIE2PDEstability_2D(PIE);
    else
        [prog, P] = PIETOOLS_FEPIE2PDEstability_2D(PIE,settings);
    end
    return
end
...
if nargin<2
    ...
    settings.epneg = 1e-5;                % Negativity of derivative of Lyapunov Function in both ODE and PDE state -  Needs to be >0 
end  
...
Dop = Qop*Aop+Aop'*Qop' +epneg*(Top'*Top); 
