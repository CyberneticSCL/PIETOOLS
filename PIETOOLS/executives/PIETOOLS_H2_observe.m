...
ny=PIE.C2.dim(1,1);                % retrieve the number of real-valued observed outputs
...
[prog,Wop] = lpivar(prog,[PIE.C2.dim(:,1),PIE.C2.dim(:,1)],X,ddZ);
...
Dop = [(PIE.A'*Pop+Zop*PIE.C2)*(PIE.T)+PIE.T'*(PIE.A*Pop+Zop*PIE.C2)'+PIE.C1'*PIE.C1]; 
...
if sosineq_on
    ...
    [prog, De3op] = poslpivar(prog, Iz.dim(:,1)+PIE.T.dim(:,1),X,dd2,options2);
    
    if override2~=1
        [prog, De2op] = poslpivar(prog,PIE.T.dim(:,1),X, dd3,options3);
        [prog, De4op] = poslpivar(prog, Iz.dim(:,1)+PIE.T.dim(:,1),X,dd3,options3);
        Deop=De1op+De2op;
        Deop2=De3op+De4op;
    else
        Deop=De1op;
