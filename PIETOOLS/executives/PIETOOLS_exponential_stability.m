...
function [prog, P,decayrate] = PIETOOLS_exponential_stability(PIE, settings)
...
% dpvar normP;
% prog = sosdecvar(prog, normP);
% prog = sossetobj(prog, normP);

disp('- Constructing the Negativity Constraint...');
% opvar tmp; tmp.dim = Pop.dim;
% tmp.I = Pop.I; tmp.var1 = Pop.var1; tmp.var2 = Pop.var2;
% tmp.R.R0 = normP*eye(size(Pop.R.R0));
% tmp.P = normP*eye(size(Pop.P));

Dop = [PIE.T'*Pop*PIE.A+PIE.A'*Pop*PIE.T+2*epneg*PIE.T'*Pop*PIE.T];     
% Dop2 = [tmp Pop; Pop tmp];
...
%     [prog, De3op] = poslpivar(prog, 2*PIE.T.dim(:,1),X,dd2,options2);
    prog = lpi_eq(prog,Dop+Deop,'symmetric'); %Dop=-Deop
%     prog = lpi_eq(prog,Dop2-De3op,'symmetric'); %Dop=-Deop
end
...
% Conclusion:
% decayrate = epneg/(2*sosgetsol(prog,normP));
