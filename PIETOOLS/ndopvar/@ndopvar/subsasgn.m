...
switch L(1).type
    ...
    case '()'
        ...
        if isa(RHS, 'nopvar') % if RHS is nopvar, convert it to ndopvar
            a_dvarname = a.dvarname;
            RHS = change_dec_var(RHS, a_dvarname); % convert to the same dec var
        end
        % if operators have different decvar, convert to the same decvar
        if isa(a, 'ndopvar')
            Aop_dvarname = string(a.dvarname);
            Aop_dvarname = strtrim(Aop_dvarname);
        else
            Aop_dvarname = {};
        end
        if isa(RHS, 'ndopvar')
            Bop_dvarname = string(RHS.dvarname);
        ...
        if numel(Aop_dvarname) ~= numel(Bop_dvarname) || ~isequal(Aop_dvarname,Bop_dvarname)
            ...
            RHS = change_dec_var(RHS, full_dvars); % convert to the same dec var
        end
...
% choosing columns of (I otimes Z_1 otimes ... otimes Z_2 ) otimes (I otimes xi)
        Loc_L_R = (Loc_L_R - 1)*(1+ length(dec_var_NEW)) + 1;
        Loc_L_R= [kron(Loc_L_R, ones(length(dec_var_NEW) + 1, 1)), kron( ones(length(Loc_L_R), 1), (0:(length(dec_var_NEW)))' )];
        Loc_L_R = sum(Loc_L_R, 2);
        ...
        % end
    case '{}'
        error('{}- like subsassign is not supported for ndopvar objects.');
end
