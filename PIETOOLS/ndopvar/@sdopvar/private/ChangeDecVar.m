...
% this routine does not change the operator P. It only rewrites it so that B colummns match a new decision-variable ordering Zd,
...
% old number of columms in each B{i}.
n_old = numel(Zd_old);
% new number of columms in each B{i}.
n_new = numel(Zd_new);
params = P.params;
for ii=1:numel(params.A)
    ...
    if isempty(Bii)
        %Treats an empty B as a sparse matrix with zero decision-variable collumns
        % and one row per scalar entry of Aii.
        Bii = sparse(0,numel(Aii));
    end
    block = numel(Aii);
    % Creates a new zero sparse B matrix with one column per new decision variable.
