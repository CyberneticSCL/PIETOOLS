...
s.type = '.'; s.subs = 'multiplier';
Blens = cellfun('size',subsref(B,s),1);
Alens = cellfun('size',subsref(A,s),1);
P = zeros(sum(Blens),sum(Alens));
blen = [0;cumsum(Blens)]+1; alen = [0;cumsum(Alens)]+1;
[~,idx] = ismember(B,A);
