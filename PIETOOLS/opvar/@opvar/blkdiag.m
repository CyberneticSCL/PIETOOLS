...
% 
% OUTPUTS
% - Fdiag:      'opvar' object with each of the parameters definined as the
...
if nargin==1
    ...
    elseif isempty(B)
        ...
        if ~isa(A,'opvar') || ~isa(B,'opvar')
            error("Block-diagonal concatenation of opvar and non-opvar objects is currently not supported.")
        end
        if any(any(A.I~=B.I))
            error('Spatial domain of opvar objects must match for block-diagonal concatenation');
        elseif any(~isequal(A.var1,B.var1)) || any(~isequal(A.var2,B.var2))
            error('The spatial variables of opvar objects must match for block-diagonal concatenation');
        end
        O1 = opvar();
