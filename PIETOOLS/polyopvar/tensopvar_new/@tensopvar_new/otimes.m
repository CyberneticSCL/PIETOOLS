...
if all(type==A.type) && all(type==B.type)
    ...
    if A.type(1)
        ...
        A_depmat1 = sum(A.depmat1,2);
    else
        % Pointwise product along primary variable
        A_depmat1 = max(A.depmat1,[],2);
    end
    if B.type(1)
        B_depmat1 = sum(B.depmat1,2);
    else
        B_depmat1 = max(B.depmat1,[],2);
    end
    if A.type(2)
        % Tensor product along primary variable
        A_depmat2 = sum(A.depmat2,2);
    else
        % Pointwise product along primary variable
        A_depmat2 = max(A.depmat2,[],2);
    end
    if B.type(2)
        % Tensor product along primary variable
        B_depmat2 = sum(B.depmat2,2);
    else
        % Pointwise product along primary variable
        B_depmat2 = max(B.depmat2,[],2);
    end
    C.depmat1 = [A_depmat1,B_depmat1];
    C.depmat2 = [A_depmat2,B_depmat2];
    C.type = type;
end
