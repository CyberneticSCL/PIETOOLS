%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% settings_PIETOOLS_custom_2D.m     PIETOOLS 2022a
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function settings = settings_PIETOOLS_custom_2D()
% Custom file for defining settings to solve LPIs in two spatial variables.
%
...
if nargin>0
    error(['The settings_custom function does not take inputs.',...
...
d2 = {[0,1;1,2],...             % Degree of Z2^oo(x,y) in [-, y; x, x*y]
      [0,1,1,2;1,2,2,4],...     % Degree of Z2^oa(x,y,nu) in [-, y, nu, nu*y; x, x*y, x*nu, x*y*nu]
      [0,1,1,2;1,2,2,4];...     % Degree of Z2^ob(x,y,nu) in [-, y, nu, nu*y; x, x*y, x*nu, x*y*nu]
      [0,1;1,2;1,2;2,4],...     % Degree of Z2^ao(x,y,tt) in [-, y; x, x*y; tt, y*tt; x*tt, x*y*tt]
      [0,1,1,2;                 % Degree of Z2^aa(x,y,tt,nu) in [-,    y,      nu,      y*nu;
       1,2,2,3;                 %                                x,    x*y,    x*nu,    x*y*nu;
       1,2,2,3;                 %                                tt,   y*tt,   tt*nu,   y*tt*nu;
       2,3,3,4],...             %                                x*tt, x*y*tt, x*tt*nu, x*y*tt*nu]
      [0,1,1,2; 1,2,2,3; 1,2,2,3; 2,3,3,4];...      % Degree of Z2^ab(x,y,tt,nu)
      [0,1;1,2;1,2;2,4],...     % Degree of Z2^bo(x,y,tt) in [-, y; x, x*y; tt, y*tt; x*tt, x*y*tt]
      [0,1,1,2; 1,2,2,3; 1,2,2,3; 2,3,3,4],...      % Degree of Z2^ba(x,y,tt,nu)
      [0,1,1,2; 1,2,2,3; 1,2,2,3; 2,3,3,4]};        % Degree of Z2^bb(x,y,tt,nu)
...
settings.LF_opts.sep = [0;  % Set 1 to enforce Rxx{2}=Rxx{3}
                        ...
                        0]; % Set 1 to enforce R22{2,2}=R22{2,3}, R22{3,2}=R22{3,3}
