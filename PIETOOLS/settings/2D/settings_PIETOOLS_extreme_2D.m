%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% settings_PIETOOLS_extreme_2D.m     PIETOOLS 2022a
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function settings = settings_PIETOOLS_extreme_2D()
% Settings file for very simple LPIs in two spatial variables.
%
...
if nargin>0
    error(['The settings_extreme function does not take inputs.',...
...
dx = {0;            % Degree of Zx^o(x) in x
      [1;1;1];      % Degree of Zx^a(x,tt) in x, tt and (x*tt)
      [1;1;1]};     % Degree of Zx^b(x,tt) in x, tt and (x*tt)
  
% Monomials in just the second spatial variable y
dy = {0,...         % Degree of Zy^o(y) in y
      [1,1,1],...   % Degree of Zy^a(y,nu) in y, nu and (y*nu)
      [1,1,1]};     % Degree of Zy^b(y,nu) in y, nu and (y*nu)
  
% Monomials in both spatial variables
d2 = {[0,0;0,0],...             % Degree of Z2^oo(x,y) in [-, y; x, x*y]
      [0,1,1,1;0,1,1,1],...     % Degree of Z2^oa(x,y,nu) in [-, y, nu, nu*y; x, x*y, x*nu, x*y*nu]
      [0,1,1,1;0,1,1,1];...     % Degree of Z2^ob(x,y,nu) in [-, y, nu, nu*y; x, x*y, x*nu, x*y*nu]
      [0,0;1,1;1,1;1,1],...     % Degree of Z2^ao(x,y,tt) in [-, y; x, x*y; tt, y*tt; x*tt, x*y*tt]
      [0,1,1,1;                 % Degree of Z2^aa(x,y,tt,nu) in [-,    y,      nu,      y*nu;
       1,2,2,2;                 %                                x,    x*y,    x*nu,    x*y*nu;
       1,2,2,2;                 %                                tt,   y*tt,   tt*nu,   y*tt*nu;
       1,2,2,2],...             %                                x*tt, x*y*tt, x*tt*nu, x*y*tt*nu]
      [0,1,1,1; 1,2,2,2; 1,2,2,2; 1,2,2,2];...      % Degree of Z2^ab(x,y,tt,nu)
      [0,0;1,1;1,1;1,1],...     % Degree of Z2^bo(x,y,tt) in [-, y; x, x*y; tt, y*tt; x*tt, x*y*tt]
      [0,1,1,1; 1,2,2,2; 1,2,2,2; 1,2,2,2],...      % Degree of Z2^ba(x,y,tt,nu)
      [0,1,1,1; 1,2,2,2; 1,2,2,2; 1,2,2,2]};        % Degree of Z2^bb(x,y,tt,nu)
...
settings.LF_opts_psatz{1}.sep = settings.LF_opts.sep;
settings.LF_opts_psatz{1}.exclude = settings.LF_opts.exclude;
...
% Otherwise, use the same settings as for P
settings.LF_opts_psatz{2}.sep = settings.LF_opts.sep;
settings.LF_opts_psatz{2}.exclude = settings.LF_opts.exclude;
...
% above).
settings.Zop_opts.isscalar = 1;     % Set to 1 for constant parameters

% We can also require Zop to be a multiplier operator, excluding any
% integrator terms. 
settings.Zop_opts.ismultiplier = 1; % Set to 1 to exclude integrators
