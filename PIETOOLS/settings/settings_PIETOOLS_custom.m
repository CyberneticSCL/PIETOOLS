...
settings.options1.sep = 1; %this is to select separable case, R1=R2
...
if sosineq_on
    ...
    settings.options3.exclude = [0 1 0 0]; % using the LU or UL decomposition in the psatz term seems to have minimal impact when norder2=1 norder1=2
    settings.dd3 = {n_order1+Dup, [n_order2+Dup, n_order3+Dup, n_order4+Dup], [n_order2+Dup, n_order3+Dup, n_order4+Dup]};
