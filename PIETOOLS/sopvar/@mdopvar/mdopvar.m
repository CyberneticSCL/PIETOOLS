classdef (InferiorClasses={?polynomial,?sopvar,?sdopvar,?mopvar}) mdopvar
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MDOPVAR  Container for PI operators between concatenated mixed L2 spaces
...
%
% For a purely fixed container use 'mopvar'. The two stand to each other as
% 'sopvar' does to 'sdopvar': 'mopvar' is closed under composition,
% 'mdopvar' is not, since the product of two operators affine in the
...
% all as documented in 'mopvar'; this class differs from it only by
% admitting 'sdopvar' blocks and by carrying P.Zd. The validation itself is
% shared, in 'derive_mopvar_meta'.
%
...
%
% See also MOPVAR, VERIFY, SIZE, PLUS, CTRANSPOSE, MTIMES, SDOPVAR.
%
...
%
% Initial coding MMP, 09/17/2026. Split out of 'mopvar', which until this
%                date held both cases in one class, so that the containers
%                mirror the sopvar/sdopvar pair and 'class(P)' states
%                whether decision variables are present. 'mopvar' declared
...
%                there and the decision case lives here.
    ...
    methods
        function P = mdopvar(varargin)
            % P = MDOPVAR(C) builds a container from an M x N cell of
            ...
            %
            % P = MDOPVAR(Q) promotes a 'mopvar' Q, keeping its blocks and
            % metadata and leaving Zd empty.
            %
            % P = MDOPVAR(C,meta) trusts 'meta' and skips validation, used
            % by the class methods, which already know their result's
            % metadata and must not repeat the O(M*N*q) Zd check.
            %
            % MDOPVAR P1 P2 declares empty containers in the caller.
            if nargin==0
                return
            end
            if nargout==0 && (ischar(varargin{1}) || isstring(varargin{1}))
                for i = 1:nargin
                    assignin('caller',char(varargin{i}),mdopvar());
                end
                return
            end
            if isa(varargin{1},'mopvar')
                Q = varargin{1};
            ...
            C = varargin{1};
            if ~iscell(C)
                error('mdopvar:badBlocks','Blocks must be an M x N cell array.')
            end
            if nargin>=2
                meta = varargin{2};
            else
                meta = derive_mopvar_meta(C,'mdopvar',{'sopvar','sdopvar'});
