function outmopvar = canonicalize(inmopvar)
% canonicalize
%
...
%
% The native block representation inmopvar.C is preserved unchanged.
%
% The auxiliary representations are stored in
%
%   outmopvar.leftCommonBasis
%   outmopvar.leftCommonC
%
%   outmopvar.rightCommonBasis
%   outmopvar.rightCommonC
...
%
% commonRowBasis returns a mopvar whose C blocks have been rewritten
% onto common left bases, while leaving their right bases unchanged.
% ---------------------------------------------------------------
L = commonRowBasis(inmopvar);

outmopvar.leftCommonBasis = L.leftCommonBasis;
outmopvar.leftCommonC     = L.C;

% ---------------------------------------------------------------
% Build column-common representation.
%
% IMPORTANT: start again from inmopvar, not from L. This keeps the
% column-common representation independent of the row-common one.
% ---------------------------------------------------------------
R = commonColumnBasis(inmopvar);

outmopvar.rightCommonBasis = R.rightCommonBasis;
outmopvar.rightCommonC     = R.C;

end
function Q = commonRowBasis(P)
% commonRowBasis
%
% Q = commonRowBasis(P) rewrites each block row of the mopvar P so that
...
%   Q.leftCommonBasis{i}.
%
% Row consistency is already guaranteed by the mopvar constructor, so
...
% commonColumnBasis
%
% Q = commonColumnBasis(P) rewrites each block column of the mopvar P so
...
%
% Column consistency is already guaranteed by the mopvar constructor, so
...
if ~all(tf)
    error('mopvar:basisEmbedding','Old basis is not contained in new basis.')
end
