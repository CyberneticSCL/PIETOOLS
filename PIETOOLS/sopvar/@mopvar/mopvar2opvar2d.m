function Pop = mopvar2opvar2d(Pmop,vars_xy)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% POP = MOPVAR2OPVAR2D(PMOP) takes a 'mopvar' container over the spaces
% R x L2[x] x L2[y] x L2[x,y] and returns the equivalent 'opvar2d'.
%
% POP = MOPVAR2OPVAR2D(PMOP,VARS_XY) names which registry variable plays x
% and which plays y, as a 1 x 2 cellstr.
%
% INPUTS
% - Pmop:     'mopvar' over at most two spatial variables, whose output and
...
%     R20 R2x R2y R22
%
% See also OPVAR2D2MOPVAR, SOPVAR2OPVAR2D, MOPVAR2OPVAR, MOPVAR2NOPVAR.
%
% For support, contact M. Peet, Arizona State University at mpeet@asu.edu

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - mopvar2opvar2d
%
...
if ~isa(Pmop,'mopvar')
    error('mopvar2opvar2d:badInput','Input must be a mopvar object.')
end
if numel(Pmop.vars)>2
    error('mopvar2opvar2d:tooManyVars',...
        ['''opvar2d'' is the 2D class; this container has %d spatial '...
         'variables (%s). Use ''mopvar2nopvar''.'],...
...
if ~all(ismember(Pmop.vars,vars_xy))
    error('mopvar2opvar2d:varsMismatch',...
...
if numel(unique(ri))~=M || numel(unique(ci))~=N
    error('mopvar2opvar2d:repeatedSpace',...
...
for i = 1:nsp
    ...
    if sum(mask(i,:))>2
        error('mopvar2opvar2d:tooManyVarsInSpace',...
