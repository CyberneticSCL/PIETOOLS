...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% C = MTIMES(A,B) composes two 'mopvar' objects,
%
...
%
% INPUTS
% - A:      M x K 'mopvar', or a scalar double;
% - B:      K x N 'mopvar', or a scalar double; the input spaces of A must
...
% @mdopvar/mtimes, which does batch.
%
% 'mopvar' is closed under composition. A decision container is not, which
% is why @mdopvar/mtimes refuses two decision factors.
%
...
%
% See also PLUS, CTRANSPOSE, MOPVAR, MDOPVAR.
%
% For support, contact M. Peet, Arizona State University at mpeet@asu.edu

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - mtimes(mopvar)
%
...
if isnumeric(A) || isnumeric(B)
    ...
    if ~isscalar(a)
        error('mtimes:nonScalarNumeric',['A numeric factor must be scalar; '...
            'multiplication by a matrix changes the spaces and must be a mopvar.'])
    end
    Cc = P.C;
    for ii = 1:numel(Cc)
        if ~isempty(Cc{ii}),    Cc{ii} = a*Cc{ii};      end
    end
    C = mopvar(Cc,metadata(P));
    return
end

if ~isa(A,'mopvar') || ~isa(B,'mopvar')
    error('mtimes:badInput','Factors must be mopvar objects, or one a scalar.')
end
