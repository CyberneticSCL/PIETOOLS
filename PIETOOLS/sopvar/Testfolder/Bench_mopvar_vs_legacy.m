function results = Bench_mopvar_vs_legacy(which_bench)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% RESULTS = BENCH_MOPVAR_VS_LEGACY(WHICH) times and measures the memory of
% 'mopvar'/'mdopvar' operations against the equivalent operations on the
...
% A 'mopvar' is an M x N grid of 'sopvar' blocks over mixed L2 spaces, so
% each legacy class corresponds to a particular grid:
%
%   A  opvar    R^m x L2^n[s]                         -> 2 x 2 mopvar
%   B  opvar2d  R x L2[x] x L2[y] x L2[x,y]           -> 4 x 4 mopvar
%   C  nopvar   L2^n[s_1,...,s_N], no R component     -> 1 x 1 mopvar
%   D  ndopvar  same, with decision variables         -> 1 x 1 mdopvar
%
% EQUIVALENCE
...
% A uses 'opvar2mopvar', B 'opvar2d2mopvar', C 'nopvar2mopvar' and D
...
% components blockwise was not supported; 'opvar2d2mopvar' (09/18/2026)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - Bench_mopvar_vs_legacy
%
...
assert_single('mopvar');    assert_single('sopvar');    assert_single('nopvar');
...
%% ====================== A: opvar vs 2x2 mopvar ======================
function R = bench_A(R,NREP)
fprintf('\n===== A: opvar (R^m x L2^n[s]) vs 2x2 mopvar =====\n');
...
% equivalent 2x2 'mopvar'. The four components are generated as four
...
%% ====================== C: nopvar vs 1x1 mopvar ======================
function R = bench_C(R,NREP)
fprintf('\n===== C: nopvar (L2^n over N variables) vs 1x1 mopvar =====\n');
fprintf('%-14s %-11s %10s %10s %8s %11s %11s %7s %7s %6s\n',...
    'size','op','t_old','t_new','t_n/o','M_old(whos)','M_new(whos)','M_n/o','Mreal','equal');
for N = 1:3
    for deg = [1 2]
...
% One random nopvar and the equivalent 1x1 mopvar. With integral_only, the
...
%% ==================== D: ndopvar vs 1x1 mdopvar =====================
function R = bench_D(R,NREP)
fprintf('\n===== D: ndopvar vs 1x1 mdopvar, over decision variables =====\n');
fprintf('%-14s %-11s %10s %10s %8s %11s %11s %7s %7s %6s\n',...
    'size','op','t_old','t_new','t_n/o','M_old(whos)','M_new(whos)','M_n/o','Mreal','equal');
...
for N = 1:2
    for q = [1e3 1e4 1e5]
        ...
        Amd = mdopvar({ndopvar2sdopvar(And)});
        Bmd = mdopvar({ndopvar2sdopvar(Bnd)});
...
for n = [1 2]
    for deg = [1 2]
        ...
        % Genuinely EQUAL operators now, via 'opvar2d2mopvar'. Before that
        % converter existed this bench could only compare equi-dimensional
        % operators and claimed no equivalence.
        Am = opvar2d2mopvar(A2);
        Bm = opvar2d2mopvar(B2);
...
w = which(nm,'-all');
if numel(w)>1
    error('Bench_mopvar_vs_legacy:shadowed',...
...
fprintf('\n===== summary: mopvar/mdopvar cost relative to the legacy class =====\n');
