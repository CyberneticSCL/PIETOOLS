function Test_mopvar()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_MOPVAR exercises the 'mopvar' container: construction and validation,
...
Az = mopvar(mkgrid(S,dm,out_A,din_A,qA,pA,1,occ));
info = verify(Az);
...
[np,nf] = chkerr(np,nf,'conflicting domain for a shared variable',...
    'mopvar:domConflict',@() mopvar(bad_dom(S,dm)));
[np,nf] = chkerr(np,nf,'row maps into two different spaces',...
    'mopvar:rowSpaceMismatch',@() mopvar(bad_rowspace(S,dm)));
[np,nf] = chkerr(np,nf,'row has two different output dimensions',...
    'mopvar:rowDimMismatch',@() mopvar(bad_rowdim(S,dm)));
[np,nf] = chkerr(np,nf,'column maps out of two different spaces',...
    'mopvar:colSpaceMismatch',@() mopvar(bad_colspace(S,dm)));
[np,nf] = chkerr(np,nf,'entirely empty row',...
    'mopvar:emptyRow',@() mopvar({mkblk(S,dm,2,2,3,3,1),[];[],[]}));
[np,nf] = chkerr(np,nf,'non-operator block',...
    'mopvar:badBlockClass',@() mopvar({mkblk(S,dm,2,2,3,3,1),42}));
[np,nf] = chkerr(np,nf,'block carrying a dims matrix',...
    'mopvar:blockDimsNotScalar',@() mopvar({blkdims_matrix()}));
...
[np,nf] = chk(np,nf,'row with per-column vars.out order is accepted',...
    verify(mopvar(mixed_order(S,dm))).true==1);

%% ---------------------------------------------------------------- 3
fprintf('\n-- 3. plus, against apply_sopvar --\n');
B = mopvar(mkgrid(S,dm,out_A,din_A,qA,pA,2,true(2,3)));
AB = A + B;
info = verify(AB);
...
qG = pA;                pG = [2 3];
G = mopvar(mkgrid(S,dm,out_G,din_G,qG,pG,3,true(3,2)));
AG = A*G;
info = verify(AG);
...
fprintf('\n===== %d passed, %d FAILED =====\n\n',np,nf);
if nf>0
    error('Test_mopvar:failures','%d check(s) failed.',nf);
end
end
