function Test_mopvar_converters()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_MOPVAR_CONVERTERS exercises 'rand_mopvar', 'opvar2mopvar' and
% 'mopvar2opvar'.
%
% VERIFICATION APPROACH - WHY THERE IS NO ROUND TRIP HERE
% 'opvar2mopvar' and 'mopvar2opvar' are mutually inverse, so a round-trip
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - Test_mopvar_converters
%
...
rng(1);
P = rand_mopvar(spaces,dims,dom,2,0.7);
info = verify(P);
...
Pz = rand_mopvar(spaces,dims,dom,2,0.7,[true false;true true]);
...
[np,nf] = chkerr(np,nf,'an all-empty row is rejected',...
    'rand_mopvar:emptyRow',@() rand_mopvar(spaces,dims,dom,2,0.7,[false false;true true]));
...
%% ---------------------------------------------------------------- 2
fprintf('\n-- 2. opvar2mopvar, against the operator action --\n');
for n = [1 3]
    for deg = [1 2]
        m = 2;
...
%% ---------------------------------------------------------------- 3
fprintf('\n-- 3. mopvar2opvar, against the operator action --\n');
for n = [1 3]
    for deg = [1 2]
        m = 2;
        ...
        rng(700+10*n+deg);
        Pm  = rand_mopvar(sp,dm,dom,deg,0.7);
        Pop = mopvar2opvar(Pm);
...
rng(9);
P2 = rand_mopvar(sp2,dm2,dom,1,0.7);
[np,nf] = chkerr(np,nf,'two spatial variables rejected',...
    'mopvar2opvar:tooManyVars',@() mopvar2opvar(P2));
[np,nf] = chkerr(np,nf,'non-opvar input rejected',...
    'opvar2mopvar:badInput',@() opvar2mopvar(42));

%% ---------------------------------------------------------------- 5
fprintf('\n-- 5. opvar2d2mopvar / mopvar2opvar2d --\n');
...
for nn = [1 2]
    ...
    C2a = mopvar2opvar2d(Am*Bm);
    C2b = mopvar2opvar2d(Am)*mopvar2opvar2d(Bm);
    eh = 0;
    for k = 1:numel(nm2),   eh = max(eh,cmp_component(C2a.(nm2{k}),C2b.(nm2{k})));  end
    [np,nf] = chk(np,nf,...
        sprintf('n=%d: mopvar2opvar2d commutes with mtimes, err %.2e',nn,eh),eh<tol);
end
[np,nf] = chkerr(np,nf,'three variables rejected by mopvar2opvar2d',...
    'mopvar2opvar2d:tooManyVars',@() mopvar2opvar2d(rand_mopvar(...
        struct('out',{{ {'s1','s2','s3'} }},'in',{{ {'s1','s2','s3'} }}),...
        struct('out',1,'in',1),dom,1,0.7)));

%% ---------------------------------------------------------------- 6
fprintf('\n-- 6. nopvar2mopvar / mopvar2nopvar --\n');
for N = 1:2
    ...
    [np,nf] = chk(np,nf,sprintf('N=%d: conversion commutes with plus',N),...
        grids_match(Pm+Qm,nopvar2mopvar(Pn+Qn),tol));
end
% Guards.
[np,nf] = chkerr(np,nf,'a 2x2 grid has no nopvar equivalent',...
    'mopvar2nopvar:badGrid',@() mopvar2nopvar(P));
sp3 = struct('out',{{ {'s1'} }},'in',{{ {'s2'} }});
[np,nf] = chkerr(np,nf,'differing in/out spaces rejected',...
    'mopvar2nopvar:spaceMismatch',@() mopvar2nopvar(...
        rand_mopvar(sp3,struct('out',2,'in',2),dom,1,0.7)));

%% ----------------------------------------------------------------
fprintf('\n===== %d passed, %d FAILED =====\n\n',np,nf);
if nf>0
    error('Test_mopvar_converters:failures','%d check(s) failed.',nf);
end
end
...
% 'subs'; the 'mopvar' side blockwise with 'apply_sopvar'. Neither path uses
