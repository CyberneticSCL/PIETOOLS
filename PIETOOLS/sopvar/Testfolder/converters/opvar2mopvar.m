function Pmop = opvar2mopvar(Pop)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PMOP = OPVAR2MOPVAR(POP) takes an 'opvar' object POP and returns the
% equivalent 2 x 2 'mopvar' container.
%
% INPUTS
...
%
% OUTPUTS
% - Pmop:   'mopvar' over the spaces (R^m, L2^n[s]), with
%
...
% columns of a 'mopvar' cannot be entirely empty, since the container reads
% each row's space and dimension off a populated block, so an operator with
% m = 0 or n = 0 must not produce a 2 x 2 grid.
%
% See also MOPVAR2OPVAR, OPVAR2SOPVAR, OPVAR2D2MOPVAR, RAND_MOPVAR.
%
% For support, contact M. Peet, Arizona State University at mpeet@asu.edu

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - opvar2mopvar
%
...
if ~isa(Pop,'opvar')
    error('opvar2mopvar:badInput','Input must be an opvar object.')
end
d = Pop.dim;
if d(1,1)~=d(1,2) || d(2,1)~=d(2,2)
    error('opvar2mopvar:nonSquare',...
...
keep_L2 = n>0;
if ~keep_R && ~keep_L2
    error('opvar2mopvar:empty','Operator has no dimensions to convert.')
end
