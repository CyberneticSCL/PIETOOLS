function Pop = rand_mdopvar(spaces,dims,dom,deg,ndec,dnsty,occ)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% POP = RAND_MDOPVAR(SPACES,DIMS,DOM,DEG,NDEC,DNSTY,OCC) generates a random
% 'mdopvar' container over the given output and input spaces.
%
% INPUTS
% - spaces, dims, dom, deg, dnsty, occ: as for 'rand_mopvar';
...
%
% OUTPUTS
% - Pop:    'mdopvar' object whose blocks are 'sdopvar';
%
% NOTES
...
%
% For a container with NO decision variables use 'rand_mopvar'; an 'mdopvar'
...
%
% See also RAND_MOPVAR, RAND_SDOPVAR, MDOPVAR.
%
% For support, contact M. Peet, Arizona State University at mpeet@asu.edu

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PIETOOLS - rand_mdopvar
%
...
if ~isequal(size(ndec),[M,N])
    error('rand_mdopvar:badNdec','ndec must be scalar or %dx%d.',M,N)
end
...
if numel(v)~=n
    error('rand_mdopvar:badDims','%s must have %d entries.',nm,n)
end
v = v(:);
end
...
% see the note in 'rand_mopvar' on why a multi-row dom is looked up by name.
if size(dom,1)==1
    d = repmat(dom,numel(v),1);
    return
end
if ~isfield(spaces,'vars')
    error('rand_mdopvar:noVarList',...
