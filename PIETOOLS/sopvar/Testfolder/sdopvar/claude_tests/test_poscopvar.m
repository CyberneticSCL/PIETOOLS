%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_POSCOPVAR checks that the container 'poscopvar' returns really is the
...
% 'test_poscopvar_vs_poslpivar' and 'test_poscopvar_vs_poslpivar_2d' compare
...
%                  lpi_eq_mdopvar -> lpi_eq_cdopvar, mopquadvar -> copquadvar,
%                  posmopvar -> poscopvar, test_posmopvar -> test_poscopvar,
%                  test_posmopvar_vs_poslpivar -> test_poscopvar_vs_poslpivar,
%                  test_posmopvar_vs_poslpivar_2d ->
%                  test_poscopvar_vs_poslpivar_2d. File was 'test_posmopvar.m'.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for nm = {'poscopvar','copquadvar','sopquadvar','lpi_eq_cdopvar'}
    if numel(which(nm{1},'-all'))~=1
        error('test_poscopvar: %s resolves to %d files; fix the path first.',...
...
for ic = 1:size(cases,1)
    ...
    if ~isequal(size(Qcell),[N,N])
        error(['test_poscopvar: Qcell is %s for %d basis operators; the Gram '...
               'is not a single matrix spanning every pair (case %s).'],...
               mat2str(size(Qcell)),N,lbl);
    end
    if size(unique(basis_list,'rows'),1)~=N
        error('test_poscopvar: basis_list repeats a row (case %s).',lbl);
    end
    ...
    if e_id > 1e-8
        error(['test_poscopvar: the defining identity fails for case ''%s'': '...
               '<y,Px> = %.12g from the kernels but %.12g from Qcell '...
               '(relative %.3g).'],lbl,a_ker,a_fac,e_id);
    end
    if e_adj > 1e-8
        error(['test_poscopvar: the container is not self-adjoint for case '...
               '''%s'': <y,Px> = %.12g but <x,Py> = %.12g (relative %.3g).'],...
               lbl,a_ker,b_ker,e_adj);
    end
    if ~isnan(q_rel) && q_rel < -1e-9
        error(['test_poscopvar: <z,Pz> = %.3g relative to the summed block '...
...
warning(w);
fprintf('test_poscopvar passed (%d of %d cases).\n',npass,size(cases,1));
...
% nv = 1..4, and 'copquadvar' never reads prog.dom or prog.vartable anyway --
...
if numel(Kc)~=3^numel(S3)
    error('test_poscopvar: %d parameter cells for %d shared variables.',...
          numel(Kc),numel(S3));
end
...
for c = 1:N
    for e = 1:N
        ...
        if size(Q,1)~=numel(V{c}) || size(Q,2)~=numel(U{e})
            error(['test_poscopvar: Q block (%d,%d) is %s but the basis '...
...
for t = 1:nk
    ...
    switch alpha(t)
        ...
        otherwise
            error('test_poscopvar: multi-index entry %d is not in {1,2,3,4}.',alpha(t));
    end
end
...
if isfield(spec,'subset') && ~isempty(spec.subset)
    error('test_poscopvar: this test does not cover subset caps.');
end
...
if numel(c)~=nv
    error('test_poscopvar: a degree cap should be scalar or have nv entries.');
end
...
if ~all(tf(:))
    error('test_poscopvar: a decision variable named in Qcell is not in Pop.Zd.');
end
...
nQ = size(names,1);
if size(names,2)~=nQ
    error('test_poscopvar: Qcell does not describe a square matrix.');
end
M = randn(nQ,nQ);
S = (M'*M)/nQ;
[tf,loc] = ismember(names,dvars);
...
if norm(reshape(dval(loc),nQ,nQ)-S,'fro') > 1e-12*max(1,norm(S,'fro'))
    error('test_poscopvar: the naming of Q is not symmetric.');
end
