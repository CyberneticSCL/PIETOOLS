%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_POSCOPVAR_VS_POSLPIVAR_2D cross-checks 'poscopvar' against
% 'poslpivar_2d' on the FOUR spaces R^n x L_2[x] x L_2[y] x L_2[x,y].
%
% This is the configuration 'copquadvar' was written for, and a far harder
% test than the two-space 1D case in 'test_poscopvar_vs_poslpivar': there are
...
% 'test_poscopvar_vs_poslpivar' uses.
%
...
% calling 'opvar2d2copvar'. That converter is independent of everything under
% test and is separately covered by 'Test_copvar_converters' (46 checks), and
...
% 'copvar' cannot then derive those rows' space and dimension -- measured, it
...
% the remaining gap, and 'copquadvar' accepts deg.subset only for a space
...
%                  mopquadvar -> copquadvar, opvar2d2mopvar -> opvar2d2copvar,
%                  posmopvar -> poscopvar,
%                  test_posmopvar_vs_poslpivar -> test_poscopvar_vs_poslpivar,
%                  test_posmopvar_vs_poslpivar_2d ->
%                  test_poscopvar_vs_poslpivar_2d. File was
%                  'test_posmopvar_vs_poslpivar_2d.m'.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clc; clear;
rng(23);

for nm = {'poscopvar','copquadvar','poslpivar_2d','opvar2d2copvar'}
    if numel(which(nm{1},'-all'))~=1
        error('test_poscopvar_vs_poslpivar_2d: %s resolves to %d files.',...
...
%   * swapping the two middle SPACES in the poscopvar call is caught by every
...
for ic = 1:size(cases,1)
    ...
    if size(Qmat,1)~=G_pm
        error(['test_poscopvar_vs_poslpivar_2d: the Gram sides differ for case '...
               '''%s'' -- poslpivar_2d %d, poscopvar %d. The degree mapping is '...
               'wrong; the elimination code is not implicated.'],...
               lbl,size(Qmat,1),G_pm);
    end
    if numel(dv_pl)~=numel(Pop_pm.Zd)
        error(['test_poscopvar_vs_poslpivar_2d: the decision variable counts '...
               'differ for case ''%s'' -- poslpivar_2d %d, poscopvar %d, at '...
               'equal Gram size %d.'],lbl,numel(dv_pl),numel(Pop_pm.Zd),G_pm);
    end

    % % % Sample both families to rank saturation
    pts = grid_points(dom,ngrid);
    [Phi_pl,n_pl,seg] = sample_family(@(dval) opvar2d2copvar(eval_dopvar2d(Pop_pl,dv_pl,dval,NAMES)), ...
    ...
    if nbad>0
        error(['test_poscopvar_vs_poslpivar_2d: %d of %d block/cell groups '...
    ...
    if r1~=r2 || rb~=r1
        error(['test_poscopvar_vs_poslpivar_2d: the two families differ for '...
               'case ''%s'': rank(poslpivar_2d) = %d, rank(poscopvar) = %d, '...
               'rank(joint) = %d.'],lbl,r1,r2,rb);
    end
    if gap < 1e3
        error(['test_poscopvar_vs_poslpivar_2d: the singular values have no '...
...
warning(w);
fprintf('test_poscopvar_vs_poslpivar_2d passed (%d of %d cases).\n',npass,size(cases,1));


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function deg = pl2d_degrees(dx1,dx2,dy1,dy2)
% 'poslpivar_2d' degrees to 'poscopvar' degrees, over the sorted registry
% (s1,s2) = (x,y). Each space's caps are given over the whole registry and
% copquadvar uses the entries for that space's own variables, so a direction
...
% poscopvar side the container is fixed and the value is substituted when its
...
while nsamp < ndec && flat < nsat
    ...
    if numel(col)~=size(Phi,1)
        error(['test_poscopvar_vs_poslpivar_2d: sample %d has %d entries '...
...
for i = 1:M
    for j = 1:M
        B = Mop.C{i,j};
        if isempty(B)
            error(['test_poscopvar_vs_poslpivar_2d: block (%d,%d) is '...
        ...
        if numel(Kc)~=3^numel(S3)
            error(['test_poscopvar_vs_poslpivar_2d: block (%d,%d) has %d '...
