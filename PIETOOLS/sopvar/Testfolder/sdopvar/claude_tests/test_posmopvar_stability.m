%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_POSMOPVAR_STABILITY runs a stability LPI on a MIXED domain through the
% container path, and checks it against the stock opvar path on the same PIE.
%
% This is the end-to-end test for 'posmopvar': a PIE whose state lives in
...
%
%   opvar2mopvar        T and A into the container
%   posmopvar           the Lyapunov operator and the negativity operator
%   @mdopvar/mtimes     T'*P*A + A'*P*T on the mixed domain
%   @mdopvar/plus       the strict-positivity identity, and D + N
%   lpi_eq_mdopvar      the equality constraint
%
...
% 'test_posmopvar_vs_poslpivar' verifies, so the two runs pose the SAME
...
for ic = 1:numel(cases)
    ...
    [f_mo,i_mo] = run_mopvar(PIE,dd1,dd2,eppos,eppos2,sopts);

    if f_op~=f_mo
        error(['test_posmopvar_stability: the two paths disagree for case '...
               '''%s'': opvar says %s, mopvar says %s.'],lbl,i_op,i_mo);
    end
    if ~isempty(want) && f_op~=strcmp(want,'feas')
        error(['test_posmopvar_stability: case ''%s'' should be %s, but both '...
               'paths report %s.'],lbl,want,i_op);
    end

    fprintf('  passed: %-38s %-8s (opvar %s, mopvar %s)\n',lbl, ...
            ternary(f_op,'feasible','infeasible'),i_op,i_mo);
    npass = npass+1;
end

fprintf('posmopvar stability test passed (%d of %d cases).\n',npass,numel(cases));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [feas,info] = run_mopvar(PIE,dd1,dd2,eppos,eppos2,sopts)
% The same test through the container.

Tm = opvar2mopvar(PIE.T);
Am = opvar2mopvar(PIE.A);
...
prog = lpiprogram(PIE.vars(:,1),PIE.vars(:,2),PIE.dom);
[prog,Pm] = posmopvar(prog,dims,spaces,PIE.dom,pl2pm(dd1));
Imat = blkdiag(eppos*eye(n),eppos2*eye(m));
Pm = Pm + opvar2mopvar(mat2opvar(Imat,[n;m],PIE.vars,PIE.dom));
Dm = (Tm')*(Pm*Am) + (Am')*(Pm*Tm);
[prog,Nm] = posmopvar(prog,dims,spaces,PIE.dom,pl2pm(dd2));
prog = lpi_eq_mdopvar(prog,Dm+Nm,'symmetric');
evalc('sol = lpisolve(prog,sopts);');
[feas,info] = read_sol(sol);
...
% 'poslpivar' degrees to 'posmopvar' degrees. The R^n basis operator of
...
% 'test_posmopvar_vs_poslpivar', where the two families are shown to span the
