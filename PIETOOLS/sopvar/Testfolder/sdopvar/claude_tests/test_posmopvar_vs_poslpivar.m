%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TEST_POSMOPVAR_VS_POSLPIVAR cross-checks 'posmopvar' against 'poslpivar'
...
%   posmopvar   Pop = sum_kl sum_ij (Z_{alpha_i^k})* Q (Z_{alpha_j^l}) over
...
% eliminated by 'mopquadvar's own antiderivative maps rather than by
...
for ic = 1:numel(cases)
    ...
    % % % posmopvar: the R^n basis is the identity, hence degree 0 in the
    ...
    deg_pm = { struct('int',0), ...
               { struct('int',d1,'mult',0), ...
                 ...
                 struct('int',d2(1),'mult',d2(2),'joint',d2(3)) } };
    prog = lpiprogram(polynomial(vars(:)),[],dom);
    [~,Pop_pm] = posmopvar(prog,[n;m],{{},vars},dom,deg_pm);
    Kpm = posmopvar_generators(Pop_pm,vars);
    ...
    if r_pl~=r_pm || r_both~=r_pl
        error(['test_posmopvar_vs_poslpivar: the two families differ for case '...
               '''%s'': rank(poslpivar) = %d, rank(posmopvar) = %d, '...
               'rank(joint) = %d.'],lbl,r_pl,r_pm,r_both);
    end

    fprintf('  passed: %-28s dim of family = %3d  (generators: %4d poslpivar, %4d posmopvar)\n',...
            lbl,r_both,size(Phi_pl,2),size(Phi_pm,2));
    npass = npass+1;
end

fprintf('posmopvar vs poslpivar test passed (%d of %d cases).\n',npass,numel(cases));
...
% posmopvar block is a function of: Q1 multiplies the INPUT, so its argument
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function K = posmopvar_generators(Pop,vars)
% Extract the kernel of each decision variable of the 2 x 2 'mdopvar', as a
    ...
    for j = 1:2
        ...
        for q = 1:numel(Aij)
            if ~isempty(Aij{q}) && max(abs(full(Aij{q}(:))))>1e-12
                error('Expected the blocks of posmopvar to have no constant term.');
            end
        end
    end
end
...
for k = 1:ngen
    ...
    if numel(K11)~=1 || numel(K12)~=1 || numel(K21)~=1 || numel(K22)~=3
        error('Unexpected number of parameter cells in a posmopvar block.');
    end
