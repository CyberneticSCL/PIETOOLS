function [prog,Pop,Qcell,basis_list] = mopquadvar(prog,dims,spaces,dom,deg,options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [prog,Pop,Qcell,basis_list] = MOPQUADVAR(prog,dims,spaces,dom,deg,options)
% declares a self-adjoint 'mdopvar' decision operator on a CONCATENATION OF
...
%           that mopquadvar(prog,m,{'s1'},...) means what it looks like;
...
% - Pop:        M x M 'mdopvar' container representing a self-adjoint PI
...
%
% See also SOPQUADVAR, POSMOPVAR, POSSOPVAR, POSLPIVAR, SOSQUADVAR,
% INT_SEMISEP, MDOPVAR, LPI_EQ_MDOPVAR.
%
...
%                  so grows with ndec. Coefficients unchanged: 'test_posmopvar'
...
% same Zd - a class invariant of 'mdopvar'.
Zd = {};
for q = 1:numel(Qcell)
