...
opvar Bd1op D11op;
Bd1op.Q2 = K*inv(B*T)*Bd1;
Bd1op.I = X; D11op.I = X;  
D11op.P = C10*T*inv(B*T)*Bd1+int((Ca1*K+Cb1*V)*inv(B*T)*Bd1,s,a,b);

T = (Bd1op'*Pop*B1op);

Dop = [T.P+T.P'-gamma*eye(nw)   D11op'              (B1op'+Bd1op')*(Pop*Eop);
        D11op                -gamma*eye(nz)  C1op;
        Eop'*(Pop*(B1op+Bd1op))       C1op'             Eop'*(Pop*Aop)+Aop'*(Pop*Eop)]; 
