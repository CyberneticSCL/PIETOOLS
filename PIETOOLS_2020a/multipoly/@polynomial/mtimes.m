...
% 6/14/2019: Added compatibility with operator products - sachin

% if isa(b,'operator') % added by sachin
%     a = a(1);
%     A = operator();
%     A.I = b.I; A.var1 = b.var1; A.var2 = b.var2;
%     r = b.dim(1,1); p = b.dim(2,1);
%     A.dim = [size(a,1)*(b.dim(1,1)~=0) r;size(a,1)*(b.dim(2,1)~=0)  p];
%     idxr = 1:r; idxp = r+1:r+p;
%     Ar = a(:,idxr); 
%     Ap = a(:,idxp);
%     A.R.R0 = Ap; A.R.R1 = zeros(size(Ap)); A.R.R2 = zeros(size(Ap));
%     A.P = Ar;
%     c = A*b;
%     return
% end
