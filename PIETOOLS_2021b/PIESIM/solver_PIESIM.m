...
% Damped wave equation
        ...
        pvar s theta RL tau Kt mu;

PDE.n.nx = 1; PDE.n.nz = 2; PDE.n.ny = 1; PDE.n.nw = 1; PDE.n.nu = 1;
PDE.n.nv = 2; PDE.n.nr = 2; PDE.n.n_pde = [0 2 1];

PDE.ODE.A = -RL; PDE.ODE.Bxu = 1; PDE.ODE.Dzu = [0;1]; 
PDE.ODE.Cv = [Kt;0]; PDE.ODE.Dvw = [0; 1]; PDE.ODE.Dzr = [1 0; 0 0]; PDE.ODE.Dyr = [0 1];

PDE.BC.Ebv = [0 0; 0 0; 1 0; 0 1];
PDE.BC.Ebb{1}.coeff = [0 0; 1 0; 0 0; 0 0]; PDE.BC.Ebb{1}.Rstate = 1;PDE.BC.Ebb{1}.D = 0;PDE.BC.Ebb{1}.delta = 0;
PDE.BC.Ebb{2}.coeff = [1; 0; 0; 0]; PDE.BC.Ebb{2}.Rstate = 2;PDE.BC.Ebb{2}.D = 0;PDE.BC.Ebb{2}.delta = 0;
PDE.BC.Ebb{3}.coeff = [0 0; 0 0; 0 1; 0 0]; PDE.BC.Ebb{3}.Rstate = 1;PDE.BC.Ebb{3}.D = 0;PDE.BC.Ebb{3}.delta = 1;
PDE.BC.Ebb{4}.coeff = [0;0;0;1]; PDE.BC.Ebb{4}.Rstate = 2;PDE.BC.Ebb{4}.D = 1;PDE.BC.Ebb{4}.delta = 1;

PDE.BC.Ebp{1}.coeff = [0 0; 0 0; 0 0; 0 1]; PDE.BC.Ebp{1}.Rstate = 1; PDE.BC.Ebp{1}.D = 0; 

PDE.PDE.A{1}.coeff = [0 0; 0 0; 1 0]; PDE.PDE.A{1}.Lstate = 2; PDE.PDE.A{1}.Rstate = 1; PDE.PDE.A{1}.D = 0; 
PDE.PDE.A{2}.coeff = [0 0; 0 tau]; PDE.PDE.A{2}.Lstate = 1; PDE.PDE.A{2}.Rstate = 1; PDE.PDE.A{2}.D = 1;
PDE.PDE.A{3}.coeff = [1; 0]; PDE.PDE.A{3}.Lstate = 1; PDE.PDE.A{3}.Rstate = 2; PDE.PDE.A{3}.D = 2;

PDE.PDE.Crp{1}.coeff = [1; 0]; PDE.PDE.Crp{1}.Rstate = 2; PDE.PDE.Crp{1}.D = 0; 
PDE.PDE.Drb{1}.coeff = [0 0; 1 0]; PDE.PDE.Drb{1}.Rstate = 1; PDE.PDE.Drb{1}.D = 0; PDE.PDE.Drb{1}.delta = 1;
