...
% %     u(t) = x2(t,1+D),   x2_{t} = x2_{s2}        s2 in [1,1+D]
% % Then
% %
...
% %         x2(t,s2=1+D) = u(t);
% %
% % Parameters D and lam can be set.
% %---------------------------------------------------------------------% %
...
PDE_t.x{2}.vars = s2;       PDE_t.x{2}.dom = [1,1+D];
...
PDE_t.BC{3}.term{1}.loc = 1+D;      PDE_t.BC{3}.term{2}.C = -1;                   
