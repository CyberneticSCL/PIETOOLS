...
methods
            ...
            elseif (isa(varargin{1},'double') || isa(varargin{1},'polynomial')) && isnumeric(varargin{2})
                ...
                elseif ~ispvar(varargin{4}) || ~(prod(size(varargin{4}))==2) || ...
                    ...
                    if ~isempty(varargin{1})
                        ...
                        P = set(P,'R00',polynomial(mat(1:rind(1),1:cind(1))));
                        P = set(P,'Rx0',polynomial(mat(rind(1)+1:rind(2),1:cind(1))));
                        P = set(P,'Ry0',polynomial(mat(rind(2)+1:rind(3),1:cind(1))));
                        P = set(P,'R20',polynomial(mat(rind(3)+1:rind(4),1:cind(1))));
                        
                        P = set(P,'R0x',polynomial(mat(1:rind(1),cind(1)+1:cind(2))));
                        L(1).type = '.';    L(2).type = '{}';
                        L(1).subs = 'Rxx';  L(2).subs = {1};
                        P = subsasgn(P,L,polynomial(mat(rind(1)+1:rind(2),cind(1)+1:cind(2))));
                        P = set(P,'Ryx',polynomial(mat(rind(2)+1:rind(3),cind(1)+1:cind(2))));
                        L(1).subs = 'Rx2';
                        P = subsasgn(P,L,polynomial(mat(rind(3)+1:rind(4),cind(1)+1:cind(2))));
                        
                        P = set(P,'R0y',polynomial(mat(1:rind(1),cind(2)+1:cind(3))));
                        ...
                        L(1).subs = 'R2x';
                        P = subsasgn(P,L,polynomial(mat(rind(1)+1:rind(2),cind(3)+1:cind(4))));
                        L(1).subs = 'R2y';
                        P = subsasgn(P,L,polynomial(mat(rind(2)+1:rind(3),cind(3)+1:cind(4))));
                        L(1).subs = 'R22';
                        P = subsasgn(P,L,polynomial(mat(rind(3)+1:rind(4),cind(3)+1:cind(4))));
    ...
    % a opvar2d object out with out.dim = Pdim_new, if possible.
    %
    % Version 1.0
    % Date: 06/13/22
    %
    ...
    %
    % OUTPUT
    % out:  opvar2d object with Pop.dim = Pdim_new. so that
    ...
    % Initial coding DJ - 13/06/2022
        ...
        % If we're initializing an opvar2d object, don't bother checking
        ...
        if all(Pdim(:)==0)
            ...
            for k=1:numel(Rparams)
                ...
                elseif ~isa(PR,'cell')
                    ...
                    obj = set(obj,Rparams{k},polynomial(zeros(PRdim_new)),'nocheck');
                elseif isa(PR,'cell')
                    PR_new = cell(size(PR));
                    for l=1:numel(PR)
                        PR_new{l} = polynomial(zeros(PRdim_new));
                    end
        ...
        if all(dim_match(:))    % if all dimensions match, do nothing
            ...
            for k=1:numel(Rparams)
                ...
                elseif isempty(PR) || ...
                    ...
                    obj.(Rparams{k}) = polynomial(zeros(PRdim_new));
                elseif isa(PR,'cell')
                    PR_new = cell(size(PR));
                    for l=1:numel(PR)
                        PRR = PR{l};
