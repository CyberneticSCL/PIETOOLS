...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% out = set(obj,prop,val) takes in one dopvar2d object obj, and returns a
% dopvar2d object out with out.prop = val
%
% Version 2.0
% Date: 06/13/22
% 
...
% prop: char class object specifying a field name for the dopvar2d
...
%
% OUTPUT
% out:  dopvar2d object with Pop.(prop)=val
%
% NOTES:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% out = set_var1(obj,val) takes in one dopvar2d object Pop, and returns a
% dopvar2d object out with out.var1 = val.
%
% Version 1.0
% Date: 06/13/22
% 
...
%
% OUTPUT
% out:  dopvar2d object with Pop.var1 = val. Note that the parameters of Pop
...
elseif any(isequal(val(1),out.var2)) || any(isequal(val(2),out.var2))
    error('The primary variables cannot match the dummy variables in the dopvar2d object');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% out = set_var2(obj,val) takes in one dopvar2d object Pop, and returns a
% dopvar2d object out with out.var2 = val.
%
% Version 1.0
% Date: 06/13/22
% 
...
%
% OUTPUT
% out:  dopvar2d object with Pop.var2 = val. Note that the parameters of Pop
%       will be adjusted to match the new variable names. That is, e.g.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% out = set_param(obj,PR_name,PR_val) takes in one dopvar2d object Pop, and
% a dopvar2d object out with out.(PR_name) = PR_val, if possible.
%
% Version 1.0
% Date: 06/13/22
% 
...
%                      'Ry0', 'Ryx', 'Ryy', 'Ry2';
%                      'R20', 'R2x', 'R2y', 'R22'};
% PR_val:   double, polynomial, or dpvar class object if the parameter is one of
%           {'R00','R0x','R0y','R02','Rx0','Rxy','Ry0','Ryx','R20'}.
%           cell of double, polynomial or dpvar class objects otherwise. PR_val
...
%
% OUTPUT
% out:  dopvar2d object with Pop.(PR_name) = PR_val. 
...
elseif isa(PR_val,'cell')
    ...
    if ~isa(Pop.(Rparams{ii_loc,jj_loc}),'cell')
        error(['The proposed parameter "',Rparams{ii_loc,jj_loc},'" should be specified as an object of type "double", "polynomial", or "dpvar".'])
    ...
    for k=1:numel(PR_val)
        ...
        elseif isa(PR_val{k},'polynomial') || isa(PR_val{k},'dpvar')
            ismatch_vars = ismatch_vars && all(ismember(PR_val{k}.varname,Param_vars{k}.varname));
            ismatch_dim = all(size(PR_val{k})==[Pop.dim(ii_loc,1),Pop.dim(jj_loc,2)]);
        else
            error(['Each element of the proposed parameter "',Rparams{ii_loc,jj_loc},'" should be specified as an object of type "double", "polynomial", or "dpvar".'])
        end
    ...
    error(['Parameters should be specified as objects of type "double", "polynomial", or "dpvar".'])
end
...
for k=1:numel(Rparams)
    ...
    elseif isa(PR,'polynomial') || isa(PR,'dpvar')
        ...
        if all(all(isequal(PR,0))) || all(all(PR.C==0))
            iszero_param(k) = true;
        end
    ...
    elseif isa(PR,'cell')
        ...
        for l=1:numel(PR)
            ...
            elseif isa(PRR,'polynomial') || isa(PRR,'dpvar')
                nr = size(PRR,1);   nc = size(PRR,2);
                dim_out_k(l) = nr;  dim_in_k(l) = nc;
                if all(all(isequal(PRR,0))) || all(all(PRR.C==0))
                    iszero_param_k(l) = true;
                end
...
if isa(PR_val,'double') || isa(PR_val,'polynomial') || isa(PR_val,'dpvar')
    ...
    if all(size(PR_val)==dim_ij)
        ...
        if all(dim_match(~isflex_Pdim))
            ...
            % parameter
            out = dopvar2d([],Pdim_new,Pop.I,Pop.var1,Pop.var2);
            for k=1:numel(Rparams)
                [ii,jj] = ind2sub(size(Rparams),k);
...
elseif isa(PR_val,'cell')
    ...
    for l=1:numel(PR_val)
        ...
        elseif (isa(PRR,'double') || isa(PRR,'polynomial') || isa(PRR,'dpvar'))
            ...
            error('Parameters of dopvar2d object must be specified as ''double'' or ''polynomial'' class objects.');
        end
    end
    ...
    error('Parameters of dopvar2d object must be specified as ''double'', ''polynomial'', or ''dpvar'' class objects, or cells (where applicable).');
end
