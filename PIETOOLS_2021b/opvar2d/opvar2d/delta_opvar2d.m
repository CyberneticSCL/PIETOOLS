...
delvar = invar(1);      ppp = inval(1);
addvar = invar(2:end);  addval = inval(2:end);
...
if strcmp(P.var1(1).varname,delvar) || strcmp(P.var2(1).varname,delvar)
    ...
    R00 = [P.R00; subs(P.Rx0,sss,ppp)];
    DP.R00 = [R00, [zeros(size(P.R00,1),size(P.Rxx{1},2)); subs(P.Rxx{1},sss,ppp)]];
    if logval %evaluate at upper boundary
        DP.R0x = [P.R0x; subs(P.Rxx{2},[sss;ttt],[ppp;sss])];
    else
        DP.R0x = [P.R0x; subs(P.Rxx{3},[sss;ttt],[ppp;sss])];
    end
    R0y = [P.R0y; subs(P.Rxy,sss,ppp)];
    DP.R0y = [R0y, [zeros(size(P.R0y,1),size(P.Rx2{1},2)); subs(P.Rx2{1},sss,ppp)]];
    if logval
        DP.R02 = [P.R02; subs(P.Rx2{2},[sss;ttt],[ppp;sss])];
    else
        DP.R02 = [P.R02; subs(P.Rx2{3},[sss;ttt],[ppp;sss])];
    end
    
    % Build the new map to L2[y]
    Ry0 = [P.Ry0; subs(P.R20,sss,ppp)];
    DP.Ry0 = [Ry0, [zeros(size(P.Ry0,1),size(P.R2x{1},2)); subs(P.R2x{1},sss,ppp)]];
    if logval
        DP.Ryx = [P.Ryx; subs(P.R2x{2},[sss;ttt],[ppp;sss])];
    else
        DP.Ryx = [P.Ryx; subs(P.R2x{3},[sss;ttt],[ppp;sss])];
    end
    Ryy{1} = [P.Ryy{1}; subs(P.R2y{1},sss,ppp)];
    DP.Ryy{1} = [Ryy{1}, [zeros(size(P.Ryy{1},1),size(P.R22{1,1},2)); subs(P.R22{1,1},sss,ppp)]];
    Ryy{2} = [P.Ryy{2}; subs(P.R2y{2},sss,ppp)];
    DP.Ryy{2} = [Ryy{2}, [zeros(size(P.Ryy{2},1),size(P.R22{1,2},2)); subs(P.R22{1,2},sss,ppp)]];
    Ryy{3} = [P.Ryy{3}; subs(P.R2y{3},sss,ppp)];
    DP.Ryy{3} = [Ryy{3}, [zeros(size(P.Ryy{3},1),size(P.R22{1,3},2)); subs(P.R22{1,3},sss,ppp)]];
    if logval
        DP.Ry2{1} = [P.Ry2{1}; subs(P.R22{2,1},[sss;ttt],[ppp;sss])];
        DP.Ry2{2} = [P.Ry2{2}; subs(P.R22{2,2},[sss;ttt],[ppp;sss])];
        DP.Ry2{3} = [P.Ry2{3}; subs(P.R22{2,3},[sss;ttt],[ppp;sss])];
    else
        DP.Ry2{1} = [P.Ry2{1}; subs(P.R22{3,1},[sss;ttt],[ppp;sss])];
        DP.Ry2{2} = [P.Ry2{2}; subs(P.R22{3,2},[sss;ttt],[ppp;sss])];
        DP.Ry2{3} = [P.Ry2{3}; subs(P.R22{3,3},[sss;ttt],[ppp;sss])];
    end
    ...
    if nargin==4 && (strcmpi(opts,'exclude') || strcmpi(opts,'pure'))
        ...
        elseif ~isempty(exclude_c)
            error('The desired option is not allowed: Delta*P*u is not independent of Delta*u')
        end
    end
    ...
    % Build the new map to R^n
    R00 = [P.R00; subs(P.Ry0,sss,ppp)];
    DP.R00 = [R00, [zeros(size(P.R00,1),size(P.Ryy{1},2)); subs(P.Ryy{1},sss,ppp)]];
    R0x = [P.R0x; subs(P.Ryx,sss,ppp)];
    DP.R0x = [R0x, [zeros(size(P.R0x,1),size(P.Ry2{1},2)); subs(P.Ry2{1},sss,ppp)]];
    if logval %evaluate at upper boundary
        DP.R0y = [P.R0y; subs(P.Ryy{2},[sss;ttt],[ppp;sss])];
    else
        DP.R0y = [P.R0y; subs(P.Ryy{3},[sss;ttt],[ppp;sss])];
    end
    if logval
        DP.R02 = [P.R02; subs(P.Ry2{2},[sss;ttt],[ppp;sss])];
    else
        DP.R02 = [P.R02; subs(P.Ry2{3},[sss;ttt],[ppp;sss])];
    end
    
    % Build the new map to L2[x]
    Rx0 = [P.Rx0; subs(P.R20,sss,ppp)];
    DP.Rx0 = [Rx0, [zeros(size(P.Rx0,1),size(P.R2y{1},2)); subs(P.R2y{1},sss,ppp)]];
    Rxx{1} = [P.Rxx{1}; subs(P.R2x{1},sss,ppp)];
    DP.Rxx{1} = [Rxx{1}, [zeros(size(P.Rxx{1},1),size(P.R22{1,1},2)); subs(P.R22{1,1},sss,ppp)]];
    Rxx{2} = [P.Rxx{2}; subs(P.R2x{2},sss,ppp)];
    DP.Rxx{2} = [Rxx{2}, [zeros(size(P.Rxx{2},1),size(P.R22{2,1},2)); subs(P.R22{2,1},sss,ppp)]];
    Rxx{3} = [P.Rxx{3}; subs(P.R2x{3},sss,ppp)];
    DP.Rxx{3} = [Rxx{3}, [zeros(size(P.Rxx{3},1),size(P.R22{3,1},2)); subs(P.R22{3,1},sss,ppp)]];
    if logval
        DP.Rxy = [P.Rxy; subs(P.R2y{2},[sss;ttt],[ppp;sss])];
    else
        DP.Rxy = [P.Rxy; subs(P.R2y{3},[sss;ttt],[ppp;sss])];
    end
    if logval
        DP.Rx2{1} = [P.Rx2{1}; subs(P.R22{1,2},[sss;ttt],[ppp;sss])];
        DP.Rx2{2} = [P.Rx2{2}; subs(P.R22{2,2},[sss;ttt],[ppp;sss])];
        DP.Rx2{3} = [P.Rx2{3}; subs(P.R22{3,2},[sss;ttt],[ppp;sss])];
    else
        DP.Rx2{1} = [P.Rx2{1}; subs(P.R22{1,3},[sss;ttt],[ppp;sss])];
        DP.Rx2{2} = [P.Rx2{2}; subs(P.R22{2,3},[sss;ttt],[ppp;sss])];
        DP.Rx2{3} = [P.Rx2{3}; subs(P.R22{3,3},[sss;ttt],[ppp;sss])];
    end
