function sttngs = lpisettings(type,dim,derivative_strictness,simplify,solver)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%           Defaults to 'light'.
%   dim:    Scalar 1 or 2, indicating the dimension of the PIE for which to
%           solve an LPI. If dim=1, 2d settings will be added as an
%           additional field "settings_2d" to the output structure. If
%           dim=2, 1d settings will be added as an additional field
%           "settings_1d" to the output structure. 
%           Defaults to 1.
...
if dim==1
    ...
    switch type
        % Set the general LPI settings for 2D case, adding 1D settings as a
        % separate field.
        case 'light'
            sttngs = settings_PIETOOLS_light_2D;
            sttngs.settings_1d = settings_PIETOOLS_light;
        case 'custom'
            sttngs = settings_PIETOOLS_custom_2D;
            sttngs.settings_1d = settings_PIETOOLS_custom;
        case 'extreme'
            sttngs = settings_PIETOOLS_extreme_2D;      
            sttngs.settings_1d = settings_PIETOOLS_extreme;
        case 'heavy'
            sttngs = settings_PIETOOLS_heavy_2D;
            sttngs.settings_1d = settings_PIETOOLS_heavy;
        case 'stripped'
            sttngs = settings_PIETOOLS_stripped_2D;
            sttngs.settings_1d = settings_PIETOOLS_stripped;
        case 'veryheavy'
            sttngs = settings_PIETOOLS_veryheavy_2D;
            sttngs.settings_1d = settings_PIETOOLS_veryheavy;
        otherwise
            warning('Unknown settings parameter requested. Defaulting to light settings');
            sttngs = settings_PIETOOLS_light_2D;
            sttngs.settings_1d = settings_PIETOOLS_light;
    ...
    % Set strictness of positivity of LF and negativity of its derivative.  
    sttngs.eppos = [1e-4; 1e-6; 1e-6; 1e-6];    % Positivity of LF wrt R x L2[s1] x L2[s2] x L2[s1,s2]
    sttngs.epneg = derivative_strictness;       % Negativity of Derivative of Lyapunov Function in both ODE and PDE state
    
    % Set same settings for 1D case.
    sttngs.settings_1d.eppos = 1e-4;      % Positivity of Lyapunov Function with respect to real-valued states
    sttngs.settings_1d.eppos2 = 1*1e-6;   % Positivity of Lyapunov Function with respect to spatially distributed states
    sttngs.settings_1d.epneg = derivative_strictness;    % Negativity of Derivative of Lyapunov Function in both ODE and PDE state 
